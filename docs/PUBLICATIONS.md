# Publications

Papers and intellectual property connected to the Future Energy Lab's work.
Status values: **published**, **accepted**, **submitted**, **in preparation**.

## Journal / manuscript

| # | Title | Topic | Status |
|---|---|---|---|
| 1 | A Physics-Informed Neural Network Framework for Arc-Fault Detection | Arc-fault detection | Submitted |
| 2 | Comparative study of arc-fault detection methods | Arc-fault detection | in preparation |
| 3 | Hybrid LSTM–CUSUM Framework for Electricity Theft Detection in Residential Smart Grids | Smart metering / AMI | submitted |
| 4 | Time-series analysis of photovoltaic curtailment in radial distribution networks | Distribution networks | Published , [Time-series analysis of photovoltaic curtailment in radial distribution networks]|

## Conference

| # | Title | Topic | Status |
|---|---|---|---|
| 1 | Inferring Low-Voltage Distribution Topology from Trench Footprints: A Deep Learning Framework | Distribution networks | Published,[https://ieeexplore.ieee.org/document/11365432] |

## Intellectual property

| # | Description | Status |
|---|---|---|
| 1 | Patent application related to arc-fault detection | in progress |

> Update this file whenever a paper changes status. Add authors, venue, DOI, and year as
> they become available.
